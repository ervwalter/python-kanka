...
from .objects.user import Campaign, Profile
from .utils import KankaSession, append_from, create_entity, to_datetime
...
class KankaClient:
    ...
    def import_campaign(self, c_id=None):
        ...
        for entity in entitylist:
            ...
            endpoint = f"{entity}s"
            data["data"][endpoint] = append_from(
                self.session, [], f"campaigns/{str(c_id)}/{endpoint}"
            )
