...
class TestRateLimitingIntegration(IntegrationTestBase):
    ...
    def test_rate_limit_with_custom_settings(self):
        ...
        try:
            ...
            print(f"  Hit rate limit after {created_count} creations with custom settings")
            
        elapsed = time.time() - start_time
        print(f"  Created {created_count} characters in {elapsed:.1f}s with custom settings")
